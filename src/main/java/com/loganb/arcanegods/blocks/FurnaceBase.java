...
public class FurnaceBase extends BlockBase implements ITileEntityProvider {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if(!world.isRemote) {
			player.openGui(Main.instance, Reference.GUI_BRICK_FURNACE, world, pos.getX(), pos.getY(), pos.getZ());
		}
		return true;
	}
	...
	public static void setState(boolean active, World world, BlockPos pos) {
		...
		if(active) {
			world.setBlockState(pos, ModBlocks.BRICK_FURNACE.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(BURNING, true), 3);
		} else {
			world.setBlockState(pos, ModBlocks.BRICK_FURNACE.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(BURNING, false), 3);
		} 
